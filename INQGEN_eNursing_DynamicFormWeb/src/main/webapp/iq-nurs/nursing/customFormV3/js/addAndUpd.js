...
function createElement(formItemsTemplate, pageReady) {
    ...
    if (dynamicFormTemplate.verification) {
        try {
            viryRules = JSON.parse(dynamicFormTemplate.verification);
        } catch (e) {
...
function setElementValue_do($pFormItem, dfHashItems, itemMap) {
    $pFormItem.each(function () {
        ...
        if (nodeId == undefined) { //非group
            var formItem = itemMap[bean]
            if(that.name === 'evaluationTime'){
                // 評估日期(evaluationTime)元件
                formItem = {
                    "itemKey": 'evaluationTime',
                    "itemValue": gFormJS.evaluationTime,
                    "nodeId": ""
                }
            }
            dynamicForm.setElementValue(this, formItem, dfHashItems[bean], itemMap, dfHashItems);
        } else { //group
...
function gFormSave(ele, successCall, errorCall, isCheckAll) {
    ...
    if (items) {
        ...
        // evaluationTime 時間不保存在gFormItems
        // 保存是否勾選當機補書輸
        var evaluationTimeItem = ""
        for (var key in items) {
            // 尋找是否有名為evaluationTime的項目
            if (items[key].itemKey === 'evaluationTime') {
                evaluationTimeItem = items[key];
                if(evaluationTimeItem !== ""){
                    if(evaluationTimeItem.itemValue.length === 16) {
                        // 若沒有秒則補 :00.00
                        evaluationTimeItem.itemValue += ':00.000'
                    }
                    // 將值保存至gFormJS的evaluationTime中
                    gFormJS.evaluationTime = evaluationTimeItem.itemValue;
                    gFormJS.evaluationTime = gFormJS.evaluationTime.replace(/-/g, '/');
                }
                // 使用gFormJS的evaluationTime，故將欄位從items刪除
                items.splice(key, 1)

                // 當機補輸checkbox的gFormItem
                var evaluationTimeMendCheck = {
                    'itemKey': 'evaluationTimeMendCheck',
                    'itemValue': $('#evaluationTimeCheckbox').prop('checked')?'Y':'N',
                    'nodeId': ''
                }
                items.push(evaluationTimeMendCheck)
                break;
            }
        }
        ...
        gFormJS.addOrUpdateGForm(gFormJS,
            function (rsGForms) {
                ...
                for (var i = 0, len = apis.length; i < len; ++i) {
                    //表單後拋轉 or 表單正式保存後拋轉
                    if (apis[i].runMode.indexOf('B') > -1 || (apis[i].runMode.indexOf('BY') > -1 && gForm.status==='Y')) {
                        ++backApisCounts;
                    }
                }
                //調用api
    ...
    $(".formItem[data-upload='true']").each(function () {
        ...
        if (type === "checkbox") {
            ...
            if ($(this).attr("data-itemvalue")) {
                formItem.itemValue = $(this).attr("data-itemvalue").trim();
            } else {
                formItem.itemValue = this.innerHTML.trim();
            }
            if ($(this).attr("data-othervalue")) {
                formItem.otherValue = $(this).attr("data-othervalue").trim();
            }
        ...
        if (needCheck) {
            if (that.required == "true") {
                if ((!formItem.itemValue || (type === "hidden" && formItem.itemValue === " "))
                    ...
                    rejectedItems.push(rejectedItem);
                    if (!isCheckAll) return false;
                } else if (!formItem.itemValue && !formItem.otherValue && type === "csCanvas") {
                    rejectedItem = this;
                    rejectedItem.promptTips = that.prompttips;
                    rejectedItems.push(rejectedItem);
                    if (!isCheckAll) return false;
...
function countTotalScore(tar, scoreId, scoreLabelId, formToolAttribute, scoreDesc) {
    ...
    if (scoreDesc) {
        ...
        for (var i = scoreDesc.length - 1; i > -1; i--) {
            ...
            if (evalByReturn(scoreEle.arithmetic)) {
    ...
    if (formToolAttribute) {
        ...
        if (typeof scoreRule === 'string') scoreRule = JSON.parse(scoreRule);
...
function setDatetimeDafaultValue(that, defaultValue, typeFormat) {
    ...
    if (dataset.datetime == "date") {
        ...
        date.setFullYear(limit.year);
        date.setDate(1); // 避免前1個月日期較少，導致變成同一個月
...
function setDatetime_TypeformatLimit(that, baseTime) {
    var dataset = getDataset(that);
    if (dataset.datetime == "date") {
        ...
        //取出年月日的最小和最大值
        var limitYMD = getDateLimit(minLimit, maxLimit, baseTime);
        //起始日期
        dt.setFullYear(limitYMD.minYear);
        dt.setDate(1); // 避免前1個月日期較少，導致變成同一個月
        ...
        //結束日期
        dt.setFullYear(limitYMD.maxYear);
        dt.setDate(1); // 避免前1個月日期較少，導致變成同一個月
...
//設置日期時間範圍
function setDatetimeLimit(that, baseTime) {
    var dataset = getDataset(that);
    if (dataset.datetime == "date") {
        //設置紀錄日期限制
        $(that).change(function () {
            ...
            dateMin.setFullYear(limitYMD.minYear);
            dateMin.setDate(1); // 避免前1個月日期較少，導致變成同一個月
            ...
            dateMax.setFullYear(limitYMD.maxYear);
            dateMax.setDate(1); // 避免前1個月日期較少，導致變成同一個月
        ...
        $(that).change(function () {
            ...
            //取出時分的最小和最大值
            var limitHM = getTimeLimit(minLimit, maxLimit, baseTime);
            typeFormat.format = typeFormat.format.replace("hh", "HH");
            typeFormat.format = typeFormat.format.replace("ii", "mm");
            //開始判斷時間範圍
            ...
            if ($(that).hasClass("dtTimeFormItem")) { //如果是datetime的話，需要判斷日期區間
                ...
                //取出年月日的最小和最大值
                var limitYMD = getDateLimit(dtMinLimit, dtMaxLimit, baseTime);
                dateMin.setFullYear(limitYMD.minYear);
                dateMin.setDate(1); // 避免前1個月日期較少，導致變成同一個月
                dateMin.setMonth(limitYMD.minMon);
                dateMin.setDate(limitYMD.minDate);
...
/**
 * 編輯模式下取得itemKey的yyyy/MM/dd HH:mm:ss.S
 * @param {string} itemKey
 * @returns {string|null} gForm有值的話回傳yyyy/MM/dd HH:mm:ss.S，否則回傳null
 */
function getUpdPageFormItemDateTime(itemKey){
    var dateTime = null;
    if (gForm) {
        if (gForm.gformItemMap && gForm.gformItemMap[itemKey]){
            var dt = gForm.gformItemMap[itemKey].itemValue;
            if (dt && dt.trim()!=="") {
                try{
                    dateTime = new Date(dt).format('yyyy/MM/dd HH:mm:ss.S');
                }catch (e) {
        ...
        if (dateTime===null) {
            eNursing.F2ReportErrorMsg("查無dateTime資料，itemKey="+itemKey);
        }
    }
    return dateTime;
}
 ...
 */
function transDateFormat(format, baseTime) {
    var limit = {year: 0, month: 0, date: 0}, match;
    //如果格式為 -0y-0m-0d 或 +0y+0m+0d
    match = format.match(/([\+\-]\d+)(\w)([\+\-]\d+)(\w)([\+\-]\d+)(\w)/);
    if (match) {
        // 如果沒有基準日，使用現在日期
        var dt = baseTime==null?new Date(new Date().format("yyyy/MM/dd 00:00:00.000")):new Date(baseTime); //ie有時毫秒會錯有時沒有毫秒會錯
        dt = (!dt.isValid()) ? new Date(new Date().format("yyyy/MM/dd 00:00:00")) : dt;
 ...
 */
function getTimeLimit(minLimit, maxLimit, baseTime) {
    var limitHM = {minHour: 0, minMinute: 0, maxHour: 0, maxMinute: 0};
    var limit;
    //轉譯日期格式 min
    limit = transTimeFormat(minLimit, baseTime);
    limitHM.minHour = limit.hour;
    limitHM.minMinute = limit.minute;
    //轉譯日期格式 max
    limit = transTimeFormat(maxLimit, baseTime);
 ...
 */
function transTimeFormat(format, baseTime) {
    var limit = {hour: 0, minute: 0}, match;
    //如果格式為 -0y-0m-0d 或 +0y+0m+0d
    match = format.match(/([\+\-]\d+)(\w)([\+\-]\d+)(\w)/);
    if (match) {
        // 如果沒有基準日，使用現在日期
        var dt = baseTime==null?new Date(new Date().format("yyyy/MM/dd HH:mm:00.000")):new Date(baseTime); //ie有時毫秒會錯有時沒有毫秒會錯
...
fileTool.fileDefaultMultiple_show = function (beanName) {
    ...
    try {
        if (fileInfos == "") {
            ...
            fileInfos = JSON.parse(fileInfos ||fileInfos);
        }
    } catch (e) {
...
function settingCondition(that, json) {
    ...
    var re = evalByReturn(json.condition);
    if (!re.test(func)) {
...
ruleTools.viryRules = function (v, eifs) {
    ...
    for (var i2 = 0, len2 = eifs.length; i2 < len2; ++i2) {
        ...
        if (checktype == "string") {
            ...
            try {
                countsIf2[i2] = (ruleTools.func[eif.condition]) ? ruleTools.func[eif.condition](v) : evalByReturn(eif.condition + "('" + v + "')");
            } catch (e) {
...
ruleTools.func.checkTwID = function (id) {
    ...
    if (id.search(/^[A-Z](1|2)\d{8}$/i) == -1) {
        ...
        for (var i = 1; i <= 8; i++) {
            total += evalByReturn(id[i]) * (9 - i);
        }
        //補上檢查碼(最後一碼)
        total += evalByReturn(id[9]);
        //檢查比對碼(餘數應為0);
        return ((total % 10 == 0));
    }
};
//func.cn身分證字號格式的驗證 (格式正確回傳true)
ruleTools.func.checkCnID = function (v) {
...
ruleTools.func.checkTwNewResidentID = function (id) {
    ...
    for (var i = 0, len = weight.length; i < len; ++i) {
        total += (weight[i] * evalByReturn(id[i])) % 10;
    }
    return ((total % 10 == 0));
};
...
//func.tw檢查cn室話格式是否正確 (格式正確回傳true)
ruleTools.func.checkCnPhone = function (v) {
    return (/^(0\d{2,3})-?(\d{7,8})$/.test(v));
};
//func.tw檢查cn手機格式是否正確 (格式正確回傳true)
ruleTools.func.checkCnCellPhone = function (v) {
    ...
    for (var i = 0, len = idxs.length; i < len; ++i) {
        ...
        if ((operator == "||" && counts > 0) || (operator == "&&" && counts == countsIf.length)) {
            if (eval_true != undefined && eval_true != "")
                evalByRunScript(eval_true);
        } else {
            if (eval_false != undefined && eval_false != "")
                evalByRunScript(eval_false);
        }
    }
};
...
ruleTools.eval = function ($e, viryResult, json) {
    if (viryResult) {
        evalByRunScript(json.eval_true);
    } else {
        evalByRunScript(json.eval_false);
    }
};
...
var externalDataFn = {
    "frame": {},
    "successCall": function(bean, itemValue, otherValue){
        console.log('bean=', bean, 'itemValue=', itemValue, 'otherValue=', otherValue);
    },
    "errorCall_default": function(bean, e, msg){
        console.log('原#'+bean+'=', $('#'+bean).val(), '原#'+bean+'_otherText=', $('#'+bean+'_otherText').val());
        $('#'+bean).val('');
        $('#'+bean+'_otherText').val('[]');
        $('#'+bean+'_showDataDiv').empty();
        externalDataFn.errorCall(bean, e, msg);
    },
    "errorCall": function(bean, e, msg){
        console.error('bean=', bean, 'e=', e);
        alert(msg);
    },
    "cancelCall_hand": function(){
        console.log('user手動點擊取消');
    }
};
//顯示資料
externalDataFn.showData = function(ele){
    externalDataFn.showDiv(ele, 'upd');
}
/**
 * 顯示彈出框
 * @param {ele} ele - externalData元件(hidden)
 * @param {string} method - add/upd/list/print 僅add會有選擇畫面，其餘直接帶值，除非typeFormat.isShowDiv=false
 * @param {json} typeFormat -
   {
       "formType": "selectFreezingEgg",                   //調用formType-gFormWebLIST
       "isShowDiv": true,                                 //是否顯示勾選視窗，false時不顯示直接把table回傳
       "isKeepDataNewest":  "true",                       //true=使用新的資料  false=使用舊的資料(直接取otherText)
       "checkType": "checkBox",                           //勾選模式 radio/checkBox
       "paramMap": {                                      //透過URL傳參，傳參格式參考dynamicForm.getSystemValue()
           "default": {                                   //預設值
               "IDS": "externalDataItemValue",
               "HHISNUM": "",
               "SOURCE_HHISNUM": "",
               "USE_FOR": ""
           },
           "add": {                                       //點選按鈕時的傳參，將覆蓋default參數
               "IDS": "fixed:",
               "HHISNUM": "local:HHISNUM",
               "USE_FOR": "eleId:FrozenSource",
               "SOURCE_HHISNUM": "eleId:ChartNo"
           },
           "upd": {                                       //載入 編輯畫面/打印畫面 時的傳參，將覆蓋default參數
           }
       }
   }
 **/
externalDataFn.showDiv = function(ele, method, typeFormat){
    externalDataFn.closeDiv($(ele).attr('data-bean'));
    var bean = $(ele).attr('data-bean');
    //取得參數
    if (!typeFormat) {
        typeFormat = JSON.parse($(ele).attr('data-typeformat') || $(ele).attr('data-typeformat'));
    }
    console.log("========typeFormat")
    console.log(typeFormat)
    paramMap = typeFormat.paramMap;
    for (var key in paramMap["default"]) {
        if (paramMap["default"][key]==='externalDataItemValue') {
            paramMap["default"][key] = ele.value;
        }else{
            paramMap["default"][key] = dynamicForm.getSystemValue(paramMap["default"][key]);
        }
    }
    for (var key in paramMap[method]) {
        if (paramMap[method][key]==='externalDataItemValue') {
            paramMap[method][key] = ele.value;
        }else{
            paramMap[method][key] = dynamicForm.getSystemValue(paramMap[method][key]);
        }
    }
    //合併參數
    paramMap["default"] = $.extend(false, paramMap["default"], paramMap[method]);
    //拼接url傳參
    var newMultiLevel = '_externalData_'+eNursing.UUID(5);
    var url = 'gFormWebLIST.html?multiLevel='+newMultiLevel;
    for (var key in paramMap["default"]) {
        url += '&'+key+'='+encodeURI(paramMap["default"][key] || '');
    }
    //設定localStorage
    localStorage["gFormWebLIST_sourceId" + newMultiLevel] = sourceId;
    localStorage["gFormWebLIST_formType" + newMultiLevel] = typeFormat.formType;
    localStorage["gFormWebLIST_frameModel" + newMultiLevel] = 'gFormWebLIST';
    localStorage["gFormWebLIST_frameModel_INIT" + newMultiLevel] = 'gFormWebLIST_INIT';
    localStorage["extenalData_beanName" + newMultiLevel] = bean;
    //勾選模式
    localStorage["extenalData_checkType" + newMultiLevel] = typeFormat.checkType;
    //區分是否把資料顯示在extenalData元件上
    if (method==='add' && typeFormat.isShowDiv) {
        localStorage["extenalData_isShowDataToParent" + newMultiLevel] = 'false';
    } else {
        localStorage["extenalData_isShowDataToParent" + newMultiLevel] = 'true';
        //add且不顯示div的時候，要把值填上但不觸發change事件
        if (method==='add' && typeFormat.isShowDiv===false) {
            localStorage["extenalData_isSetValueToParent" + newMultiLevel] = 'true';
        }
        //如果 isKeepDataNewest=false -> 使用舊的資料
        if (method!=='add' && !typeFormat.isKeepDataNewest){
            localStorage["extenalData_otherValue" + newMultiLevel] = $('#'+bean+'_otherText').val();
        }
    }
    //add && isShowDiv=true -> 顯示，其餘隱藏
    var isShowOrHide = (localStorage["extenalData_isShowDataToParent" + newMultiLevel] === 'false')? '' : 'hide';
    //顯示div
    div =  '<div id="externalDataMaskDiv_backdrop_'+(bean||bean)+'" class="externalDataMaskDiv backdrop '+isShowOrHide+'"></div>';
    div += '<div id="externalDataMaskDiv_'+(bean||bean)+'" class="externalDataMaskDiv '+isShowOrHide+'">';
    div += '    <div class="content">';
    div += '        <div class="body">';
    div += '            <iframe id="externalDataFrame" name="externalDataFrame" class="externalDataFrame" src="'+url+'"></iframe>';
    div += '        </div>';
    div += '        <div class="footer">';
    div += '            <button class="btn btn-secondary" type="button" onClick="externalDataFn.closeDiv(\''+(bean||bean)+'\'); externalDataFn.cancelCall_hand();">關閉</button>';
    div += '        </div>';
    div += '    </div>';
    div += '</div>';
    $('body').append(div);
...
//顯示彈出框 by 按鈕
externalDataFn.showDivByBtn = function(that){
    externalDataFn.showDiv($('#'+$(that).attr('data-bean'))[0], 'add');
}
//關閉彈出框
externalDataFn.closeDiv = function(bean){
    $('#externalDataMaskDiv_backdrop_'+bean).remove();
    $('#externalDataMaskDiv_'+bean).remove();
}
//frame頁面完成
externalDataFn.frame.pageReady = function(){
    var isShowDataToParent = localStorage["extenalData_isShowDataToParent" + multiLevel] === 'true';
    var isSetValueToParent = localStorage["extenalData_isSetValueToParent" + multiLevel] === 'true';
    var bean = localStorage["extenalData_beanName" + multiLevel];
    var $showDataDiv = parent.$('#'+bean+'_showDataDiv');
    var data = [], dataDetail = [];
    $('.externalDataCheckBox').each(function(){
        data.push(this.value);
        dataDetail.push(gForms[$(this).data('index')]);
    });
    //把值填上但不觸發change事件
    if (isSetValueToParent) {
        //寫入父層的itemValue
        parent.$('#'+bean).val(data);
        parent.$('#'+bean+'_otherText').val(JSON.stringify(dataDetail));
    }
    //資料直接顯示在extenalData元件上 (不點選)
    if (isShowDataToParent) {
        parent.$('#'+bean+'_showDataDiv').empty();
        var $tableList = $('#tableList');
        $tableList.attr('id', $tableList.attr('id')+"_"+bean);
        $showDataDiv.append($tableList);
        //回調事件
        parent.externalDataFn.successCall(bean, data, dataDetail);
        //要直接顯示資料的話代表不必顯示frame
        parent.externalDataFn.closeDiv(bean);
    }
}
//取得api資料並轉成gForm格式
externalDataFn.frame.getApiDataToGForm = function(successCall){
    var errorMsg = '取得api資料發生錯誤！請洽資訊人員處理。';
    var bean = localStorage["extenalData_beanName" + multiLevel];

    //如果有值，使用舊的資料
    var extenalData_otherValue = localStorage["extenalData_otherValue" + multiLevel];
    if (extenalData_otherValue) {
        try {
            extenalData_otherValue = JSON.parse(extenalData_otherValue);
            //設定勾選模式
            if (extenalData_otherValue[0] && extenalData_otherValue[0].gForm) {
                extenalData_otherValue[0].gForm.checkType = localStorage["extenalData_checkType" + multiLevel];
            }
            successCall(extenalData_otherValue);
        }catch (e) {
            parent.externalDataFn.errorCall_default(bean, e, errorMsg);
            parent.externalDataFn.closeDiv(bean);
        }
        return;
    }
    ...
    //取得新的資料
    if (!apis || !apis[0]) {
        parent.externalDataFn.errorCall_default(bean, 'externalData元件取得api資料發生錯誤！\n尚未設定api', errorMsg);
        parent.externalDataFn.closeDiv(bean);
        return;
    }
    apis[0].complete = function(apiModule) {
        var bean = localStorage["extenalData_beanName" + multiLevel];
        //發生錯誤
        if (apiModule.resultMsg.error) {
            parent.externalDataFn.errorCall_default(bean, apiModule, apiModule.resultMsg.msg);
            parent.externalDataFn.closeDiv(bean);
            return;
        }
        //取值
        var data = [], results = apis[0].getData();
        console.log("==========results")
        console.log(results)
        try{
            for (var i=0, len=results.length; i<len; ++i){
                var gFormJson = {
                    "gForm":{
                        "formId": "formId_externalData",
                        "gformItemMap": results[i]
                    }
                };
                data.push(gFormJson);
            }
            //設定勾選模式
            if (data[0] && data[0].gForm) {
                data[0].gForm.checkType = localStorage["extenalData_checkType" + multiLevel];
            }
            successCall(data);
        }catch (e) {
            parent.externalDataFn.errorCall_default(bean, e, errorMsg);
...
//選中checkbox or tr
externalDataFn.frame.selectCheckBox = function(that){
    //doSomething...
}

//點選 "確認" 按鈕
externalDataFn.frame.clickOkBtn = function(that){
    var data = [], dataDetail = [];
    $('.externalDataCheckBox:checked').each(function(){
        data.push(this.value);
        dataDetail.push(gForms[$(this).data('index')]);
    });
    //寫入父層的itemValue
    var bean = localStorage["extenalData_beanName" + multiLevel];
    parent.$('#'+bean+'_otherText').val(JSON.stringify(dataDetail));
    parent.$('#'+bean).val(data).change();
    // bean.change()自帶close事件
    // externalDataFn.frame.clickCancelBtn();
}

//點選 "取消" 按鈕
externalDataFn.frame.clickCancelBtn = function(that){
    var bean = localStorage["extenalData_beanName" + multiLevel];
    parent.externalDataFn.cancelCall_hand();
    parent.externalDataFn.closeDiv(bean);
}
    ...
    function checkSameOptions(targetName, scriptOpts) {
        ...
        eles.each(function(index) {
            if (this.tagName.toLowerCase() === 'input') {
                ...
                checkedArray.push(selectValue)
            }
        });
        for (var j = 0, lenJ = checkedArray.length; j < lenJ; ++j) {
            for (var k = 0, lenK = scriptOpts.length; k < lenK; ++k) {
                if (scriptOpts[k] == checkedArray[j]) {
                    sameCount++;
                    break;
                }
            }

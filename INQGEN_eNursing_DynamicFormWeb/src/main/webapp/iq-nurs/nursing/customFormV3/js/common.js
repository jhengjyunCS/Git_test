...
function appendStyleToHead(context){
    //資安風險緣故，replace掉<script開頭的字樣
    $("head").append("<style>" + context.replace(/<script/g, 'systemAutoReplaceLt-script') + "</style>");
    //IE低版本不支援
    // var $style = $("<style/>");
    // $style[0].text=context;
    // $("head").append($style);
}
...
//安全版eval - 執行程式
function evalByRunScript(scriptStr){
    var script = document.createElement('script');
    script.type = 'text/javascript';
    script.text = scriptStr;
    script.classList.add("eval2");
    document.body.appendChild(script);
    var elements = document.body.getElementsByClassName('eval2');
...
/**
 * 取得json的某個節點
 * 因evalByReturn有時會受變數局域影響，造成return會報錯
 * @param {json} json
 * @param {string} nodeStr 要返回的節點，null || "" 時，returnjson
 * @returns {*}
 */
function getJsonNodeValue(json, nodeStr) {
    if (!nodeStr || nodeStr=="") return json;
    //aaa.bbb[asdsad].ccc -> ["aaa", "bbb", "asdsad", "ccc"]
    var nodeStrArr = nodeStr.match(/(\?<=\[).+(?=])|\w+/g);
    var v = json;
...
document.onkeydown=function(event){
  ...
  if(e && (((e.ctrlKey || e.metaKey) && e.keyCode==113))){
    ...
    if (pFormItemBeanName.length>0){
      ...
      $(".rdLabelFormItem, .ckLabelFormItem").each(function(){
        ...
        $(this).append("<label class='pFormItemBeanName' style='background-color: #FCF9AB;'>("+(st || st)+")</label><br class='pFormItemBeanName'/>");
      });
      //下拉框uiValue
  ...
  //顯示頁面資訊 ctrl+F1 || F2
    ...
    if (divLogF.style.display=="none"){
      ...
      divLog.innerHTML = "";
      try{
        divLog.innerHTML += "<font style='color:#5C8ADD;'>url: </font>";
        divLog.appendChild($("<label/>").text(location.href)[0] || $("<label/>").text(location.href)[0]);
        try{
          divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>sourceId: </font>";
          divLog.appendChild($("<label/>").text(sourceId)[0] || $("<label/>").text(sourceId)[0]);
        }catch(e){
          console.log(e);
        }
        try{
          divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>formId <font style='color:#50CE56;'>(GForm)</font>: </font>";
          divLog.appendChild($("<label/>").text(gFormJS.formId)[0] || $("<label/>").text(gFormJS.formId)[0]);
        }catch(e){
          console.log(e);
        }
        try{
          divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>formId <font style='color:#50CE56;'>(dynamicForm)</font>: </font>";
          divLog.appendChild($("<label/>").text(dynamicForm.formId)[0] || $("<label/>").text(dynamicForm.formId)[0]);
        }catch(e){
          console.log(e);
        }
        divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>formType: </font>";
        divLog.appendChild($("<label/>").text(formType)[0] || $("<label/>").text(formType)[0]);
        divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>frameModel: </font>";
        divLog.appendChild($("<label/>").text(frameModel)[0] || $("<label/>").text(frameModel)[0]);
        divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>frameModel_INIT: </font>";
        divLog.appendChild($("<label/>").text(frameModel_INIT)[0] || $("<label/>").text(frameModel_INIT)[0]);
        try{
          divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>versionNo: </font>";
          divLog.appendChild($("<label/>").text(versionNo)[0] || $("<label/>").text(versionNo)[0]);
        }catch(e){
          console.log(e);
        }
        if (apis && apis.length>0) {
          divLog.innerHTML += "<br/><font style='color:#5C8ADD;'>apis: </font>";
          for (var i=0, len=apis.length; i<len; ++i) {
              var $fontRunmode = $("<font style='color:#50CE56;'>");
              $fontRunmode.text((i===0? "" : ", ") + "("+apis[i].runMode+")");
              var $label = $("<label/>");
              $label.attr("title", apis[i].apiDescription);
              $label.text(apis[i].apiName);
              divLog.appendChild($fontRunmode[0]);
              divLog.appendChild($label[0]);
          }
        }
        ...
        try{
          ...
          function downloadReport(msg){
            ...
            setTimeout(function() {
              ...
              divLog.innerHTML += "<br/><br/><br/><font style='color:#5C8ADD;'>回報錯誤訊息: </font>";
...
//pageReady 之前，載入js檔案
function goPageReady(pageReady, isDontLoadPageReadyFiles){
    //若尚未到達"頁面準備完成階段"時，isDontLoadPageReadyFiles=true，不引入pageReady.js
    if (pageReady && isDontLoadPageReadyFiles) {
        pageReady();
        return;
    }
    if (pageReady){
...
function setApiSetting(dynamicFormTemplate, completeCall, isDontLoadPageReadyFiles) {
    if (dynamicFormTemplate.apiStructure) {
        ...
        for (var i = 0, len = apiStructure.length; i < len; ++i) {
            ...
            module.fn.init(JSON.parse(apiStructure[i]), function(apiModule){
                apiInitEnd(apiModule, completeCall, isDontLoadPageReadyFiles)
            }, function(error){
                apiInitError(error, completeCall, isDontLoadPageReadyFiles)
            });
        }
    } else goPageReady(completeCall, isDontLoadPageReadyFiles);
}

function apiInitEnd(apiModule, completeCall, isDontLoadPageReadyFiles) {
    apis.push(apiModule);
    try{
...
function apiInitError(error, completeCall, isDontLoadPageReadyFiles) {

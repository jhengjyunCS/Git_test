!function () {
    ...
    function DBQueryModule() {
        ...
        this.getResultSetList = function (sqlStatement, successCall, errorCall) {
            var dbQueryModule = this
            var param = {
                /**不同数据*/
                node: "resultSetList",
            ...
            eNursing.sendMsg("databaseService.getResultSetList", sqlStatement, param, "", function (result) {
                console.log(result)
                if (result.resultMsg.success) {
                    var dataObject = result.data
                    window.originResultDataObject = $.extend(true, {}, dataObject);
                    successCall(dataObject);
                } else {
                    eNursing.F2ReportErrorMsg(result);
                    errorCall(result.resultMsg.message);

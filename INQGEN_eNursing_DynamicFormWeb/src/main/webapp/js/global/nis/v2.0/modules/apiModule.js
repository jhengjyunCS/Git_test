!function () {
    function ApiModule() {
        ...
        // formId 用於更新api清單
        this.formId = null;
        // runMode 執行模式(F:前,M:中,B:後,BY:後(正式),D:後(刪除),DY:後(正式刪除))
        this.runMode = [];
        ...
        /**
         * 調用api方式
         * 目前僅開放 httpJson、apiTransAddOrUpdateGForm、apiTransDeleteGForm
             *httpJson
                -一般的api調用方式
             *apiTransAddOrUpdateGForm
                -api-gForm-拋轉 (新增/更新)
                -不必填寫url、contentType、async、dataType
                -sendParam必填api-formType、api-sourceId、parentFormId
             *apiTransDeleteGForm
                -api-gForm-拋轉 (刪除)
                -不必填寫url、contentType、async、dataType
                -sendParam必填api-formType、api-sourceId、parentFormId
         */
        this.sendMethod = "httpJson";
        /**
        ...
        */
        ...
        this.fn.setApiModule = function (apiGform) {
            ...
            thisApi.sendMethod = (map.sendMethod && map.sendMethod.itemValue) ? map.sendMethod.itemValue : 'httpJson';
            ...
            thisApi.needAutoSetting = (map.needAutoSetting && map.needAutoSetting.itemValue) ? map.needAutoSetting.itemValue : false;
        ...
        this.fn.setUrl = function() {
            if (['apiTransAddOrUpdateGForm', 'apiTransDeleteGForm'].indexOf(thisApi.sendMethod)>-1){
                return false;
            }else if (['xmlApi'].indexOf(thisApi.sendMethod)>-1){
                thisApi.connectUrl = location.origin + '/' + location.pathname.split('/')[1] + '/m2/api/gformInit'
                return false;
            }
            var apiDomain = 'cs'
            try {
            ...
            if (!thisApi.domain[apiDomain] && !thisApi.url[apiDomain]) {
                alert("api尚未設定url連線! \n\n["+thisApi.apiDescription+"] "+thisApi.apiName);
            }
        ...
        this.fn.setSendData = function () {
            ...
            //取得gForm.jsp傳參
            var gFormJspUrlParameJSON = window.localStorage['gForm.jsp_url_parameters'+multiLevel] || {};
            // 根據apiModule拼裝傳參
            sendData = deepGetSendData(thisApi.sendParam);
            function deepGetSendData(sendParam) {
                var json = {}
                for (var node in sendParam) {
                    var nodeItem    = sendParam[node];
                    //如果是[{}]的話，下鑽一層
                    if (nodeItem.type==='array' && Array.isArray(nodeItem.value) && nodeItem.value.length>0 && typeof nodeItem.value[0] ==='object') {
                        json[node] = []
                        for (var i=0, len=nodeItem.value.length; i<len; ++i){
                            json[node].push(deepGetSendData(nodeItem.value[i]));
                        }
                        continue;
                    }else if (nodeItem.type==='object' && typeof nodeItem.value ==='object' && nodeItem.source == null) {
                        json[node] = deepGetSendData(nodeItem.value);
                        continue;
                    }
                    ...
                    if (nodeSource && nodeSource.indexOf(':') > -1) {
                        ...
                        switch (nodeArray[0]) {
                            case 'local':
                                ...
                                if (localStorage[nodeArray[1]] === undefined) console.error(thisApi, failMsg + 'localStorage undefined: ' + nodeArray[1]);
                            ...
                            case 'gFormData':
                                ...
                                if (!gForm || !gForm.gformItemMap || !gForm.gformItemMap[nodeArray[1]]) {
                                    if (gForm && gForm[nodeArray[1]]) {
                                        json[node] = gForm[nodeArray[1]];
                                    } else {
                                        try {
                                            if ((typeof evalByReturn(nodeArray[1]))==='string'){
                                                json[node] = evalByReturn(nodeArray[1]);
                                            } else {
                                                throw '';
                                            }
                                        }catch (e) {
                            ...
                            case 'urlParameters':
                                // url傳參
                                json[node] = eNursing.getUrlParameter(nodeArray[1]);
                                break;
                            case 'gFormJspUrlParameters':
                                // gForm.jsp傳參
                                json[node] = gFormJspUrlParameJSON[nodeArray[1]] || '';
                                break;
                            default:
                        ...
                        if (nodeItem.type==='array') {
                            try{
                                json[node] = JSON.parse(json[node]);
                            } catch (e) {
                                console.log(failMsg + 'parse array error!', json[node], nodeItem)
                            }
                        }
                    }
                }
            ...
            thisApi.sendData = thisApi.fn.paramReplace(sendData, thisApi.sendReplacing);
        };
        ...
        this.fn.paramReplace = function (json, setting) {
            ...
            if (Array.isArray(json)) {
                for (var i=0, len=json.length; i<len; ++i) {
                    json[i] = thisApi.fn.paramReplace(json[i], setting);
                }
                return json;
            }
            //非陣列 - 開始轉換參數
        ...
        this.fn.getTargetCheck = function () {
            ...
            //轉換typeKey屬性
            for (var i=0; i<json.typeKey.length; ++i) {
                try {
                    json.typeKey[i] = thisApi.fn.changeValueType (json.typeKey[i], selector.type);
                } catch (e) {
                    console.error(failMsg + '轉換資料型態失敗(from ' + typeof json.typeKey[i] + ' to ' + selector.type + ')，不中斷繼續執行', selector, json.typeKey[i], e);
                }
            }
            //找到api訊息節點
        ...
        this.fn.getData = function (dataMapping) {
            ...
            var dataMapping = dataMapping || this.getDataMapping();
            var rs;
            ...
            ***範例回傳值
             ...
             "result2": "xxxxx"
             ***範例A dataMapping (目前只將此狀況處理為陣列 -> 只有一層陣列，且陣列節點相同)
             {
                 "a": {
                     "node": "result[0].a",
                     "type": "int",
             ...
             ***範例B dataMapping (狀況太複雜暫不處理 -> 存在兩層以上的陣列 -> result、d)
             {
                 ....abc同範例A
                 "d": {
                     "node": "result[0].c[0].d",
             ...
             ***範例C dataMapping (狀況太複雜暫不處理 -> 並非所有節點都屬於同個陣列底下)
             {
                 ....abc同範例A
                 "result2": {
                     "node": "result2",
             ...
             */
            //判斷是否只有一層陣列，且陣列節點相同
            var isArray=true, arrayNode='';
            for (var key in dataMapping) {
                //0層or多層陣列，不符範例A規則
                if (2 !== dataMapping[key].node.split("[").length){
                    isArray = false;
                    break;
                }
                arrayNode = (arrayNode==='')? dataMapping[key].node.split("[")[0] : arrayNode;
                //陣列節點不相同，不符範例A規則
                if (arrayNode !== dataMapping[key].node.split("[")[0]) {
                    isArray = false;
                    break;
                }
            }
            //從 result 中取得相關訊息
            var i=0, len=1
            rs = (isArray) ? thisApi.fn.findNode(rs, arrayNode) : [rs];
            len = rs.length;
            var json = (isArray) ? [] : {};
            for (i=i; i<len; ++i) {
                for (var key in dataMapping) {
                    var bean;
                    bean = thisApi.fn.findNode(rs[i], (isArray)? dataMapping[key].node.split(arrayNode+"[0].")[1] : dataMapping[key].node);
                    if (bean !== null) {
                        try {
                            bean = thisApi.fn.changeValueType(bean, dataMapping[key].type);
                        } catch (e) {
         ...
         * @returns {object}
         */
        this.fn.getDataGformItemMap = function (dataMapping, data) {
            var failMsg = '取得資料生成gformItemMap格式 發生錯誤 -> ';
            var json = (data) ? data : this.getData(dataMapping);

            //處理陣列的情況
            if (Array.isArray(json)) {
                for (var i=0, len=json.length; i<len; ++i) {
                    json[i] = thisApi.fn.getDataGformItemMap(dataMapping, json[i]);
                }
                return json;
            }

            //非陣列 - 從 getData( ) 取得資料後，轉換成gformItemMap格式
            for (var key in json) {
                var item = json[key];
                //轉換成string
                try{
        ...
        this.fn.getSingleApi = function (successCall, errorCall) {
            if (!gFormJS) var gFormJS = nursing.createGForm();
            gFormJS.searchParamGF.formType = thisApi.formType;
            gFormJS.searchParamGF.sourceId = thisApi.sourceId;
            gFormJS.searchParamGF.status = 'Y';
            gFormJS.searchParamGF.itemCondition = "{apiName}='"+thisApi.apiName+"'";
            ...
            //身高體重
            else if (thisApi.apiName == 'getHeightAndWeightByPatientId' || thisApi.apiName == null) {
                ...
                successCall({
                    "sourceId": "xindian",
                    "formId": "3a00bef8-377d-40b4-ab16-66054da552f9",
                    "formType": "propAPIListForm",
                    "evaluationTime": "2021/09/07 23:44:14.282",
                    "formVersionId": "f520ae09-1d00-4e4c-a9e6-210909dfd497",
                    "status": "Y",
        ...
        this.fn.comparisonObjectWithGForm = function (apiStructure, gFormObject) {
            ...
            if (!gFormItemMap.runMode) gFormItemMap.runMode = {"itemKey": "runMode", "itemValue": ""};
            ...
            if (apiStructure.runMode) gFormItemMap.runMode.itemValue = apiStructure.runMode;
        ...
        this.fn.openConnection = function (apis, successCall, errorCall) {
            ...
            if (apis.sendMethod==='apiTransAddOrUpdateGForm') {
                nursing.createGForm().apiTransAddOrUpdateGForm(apis.sendData, successCall, errorCall);
            }else if (apis.sendMethod==='apiTransDeleteGForm') {
                nursing.createGForm().apiTransDeleteGForm(apis.sendData, successCall, errorCall);
            }else if (apis.sendMethod==='xmlApi'){
                eNursing.sendMsg('xmlApi.'+apis.apiName, apis.sendData, {}, "", function(result){
                    try{
                        successCall(result.data[0]);
                    }catch (e) {
                        console.error(e, result);
                        errorCall(e);
                    }
                }, errorCall, null, null, false, apis.connectUrl, "httpJson");
            }else{
                eNursing.sendMsg('api.'+apis.apiName, apis.sendData, {}, "", function(result){
                    try{
                        successCall(result.data[0]);
                    }catch (e) {
                        console.error(e, result);
                ...
                }, errorCall, null, null, false, apis.connectUrl, apis.sendMethod || "httpJson");
            }
        };
        ...
        this.fn.doTargetCheck = function(targetCheck){
            ...
            var check = getJsonNodeValue(thisApi.result, targetCheck.checkNode);
            ...
            switch (targetCheck.typeAttr[idx]) {
                ...
                default:
                    resultMsg.other = true;
                    resultMsg.msg = "未找到狀態查核狀態值! checkNode="+"thisApi.result."+targetCheck.checkNode;
                    break;
            }
            try{
                resultMsg.msg = getJsonNodeValue(thisApi.result, targetCheck.msgNode);
                console.log(thisApi, resultMsg.msg);
            }catch (e) {
                console.error(e);
            }
            return resultMsg;
        ...
        this.fn.findAttr = function(json, targetAttr) {
            ...
            function deepFind (node, root) {
                for (var key in node) {
                    ...
                    // 如果value是物件或陣列 || 如果node本身是陣列  -> 繼續下鑽
                    if ((node[key].value && thisApi.fn.isJsonOrArrayJson(node[key].value)) || (Array.isArray(node) && thisApi.fn.isJsonOrArrayJson(node[key]))) {
                        //array要將root設為xxx[{index}]，其餘則xxx.yyy
                        deepFind((Array.isArray(node)) ? node[key] : node[key].value, (root.indexOf('{index}') > -1) ? root.replace('{index}', key) : root + (root ? '.' : '') + key + (Array.isArray(node[key].value) ? '[{index}]' : ''));
                    }
                }
            }
            return selector;
        ...
        this.fn.findNode = function(json, node) {
            try{
                return getJsonNodeValue(json, node);
            }catch (e) {
                console.error(e);
                return null;
            }
         ...
         * 轉換資料型態
         * @param v 要被轉換的資料
         * @param {string} type 要轉換的型態
         */
        this.fn.changeValueType = function (v, type) {
            try {
                switch (type) {
                    case 'string':
                        if (eNursing.isString(v)) {
                            v = v;
                        } else if (eNursing.isNumber(v)) {
                            v = v + '';
                        } else if (eNursing.isBoolean(v)) {
                            v = v + '';
                        } else if (Array.isArray(v)) {
                            v = v.join(',');
                        } else if (eNursing.isObject(v)) {
                            v = JSON.stringify(v);
                        } else {
                            throw '轉換成string時發生錯誤'
                        }
                    ...
                    case 'boolean':
                        if (eNursing.isString(v)) {
                            v = (v && (v.toLowerCase()==='true' || v==='1')) ? true : false;
                        } else if (eNursing.isNumber(v)) {
                            v = (v) ? true : false;
                        } else if (eNursing.isBoolean(v)) {
                            v = (v) ? true : false;
        ...
        this.options = {
            ...
            "ready" : {
                ...
                // api連線發生錯誤
                "openConnectionOnError" : function(apiModule, e){console.error(apiModule, e);},
                // 取得資料且處理完資料後
                ...
                "checkingOutputError" : function(apiModule, resultMsg){console.error(apiModule, resultMsg);},
        ...
        // 取得資料
        this.getData = function(dataMapping) {
            var rs;
        ...
        this.start = function(apiGform) {
            ...
            thisApi.fn.openConnection(thisApi, function(result) {
                ...
                if (thisApi.needAutoSetting && gFormJS) {
                    ...
                    // 設定完gForm後
                    readyFn.setGform(thisApi.getData(dataMapping));
                }
                thisApi.options.completeBefore(thisApi);
                thisApi.complete(thisApi);
            }, function(e){
